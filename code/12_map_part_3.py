...
MOB_STATE1 = tex_coords((2, 1), (2, 1), (0, 2))
MOB_STATE2 = tex_coords((2, 1), (2, 1), (1, 2))
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.map = pyglet.sprite.Sprite(self.map, self.width - 150, self.height - 150)
        self.map.scale = 0.275

        # player icon
        self.steve_dot = pyglet.resource.image("steve_dot.png")
        self.steve_dot = pyglet.sprite.Sprite(self.steve_dot, self.width - 85, self.height - 85)
        self.steve_dot.scale = 0.025

        # mob icon
        self.mob_dot = pyglet.resource.image("mob_dot.png")
        self.mob_dot = pyglet.sprite.Sprite(self.mob_dot, self.width - 85, self.height - 85)
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.W:
            ...
            self.model.ai.mode = "run"
            self.model.ai.status = True
            self.model.mob_loaded = False
    ...
    def draw_map(self):
        ...
        if self.model.ai.status:
            self.mob_dot.x = int(self.width - 85 + 137*(self.model.mob_x_position/160))
            self.mob_dot.y = int(self.height - 85 - 137*(self.model.mob_z_position/160))
            self.mob_dot.draw()
    ...
    def move_mob(self):
        moved_up = False
        if self.model.ai.status and self.model.ai.count >= 15:
            ...
            while (self.check_avail(pos) and pos[1] >= -1 and not moved_up):
